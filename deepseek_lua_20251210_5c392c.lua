...
-- Aimbot Settings (Menggunakan script aimbot Anda)
local Aimbot = {
    Enabled = false,
    ToggleMode = false,
    TeamCheck = false,
    ...
    -- Tambahan dari script Anda
    BlatantMode = false,
    WallCheckEnabled = true,
    
    -- Instant Reaction Shot Settings
    InstantShot = {
        Enabled = false,
        AutoShoot = false,
        ReactionTime = 0.1, -- dalam detik
        MaxDistance = 500,
        RequireWeapon = true,
        OnlyWhenAiming = false,
        ShotDelay = 0.2, -- delay antar tembakan
        LastShotTime = 0
    }
}
...
-- ============================
-- AIMBOT SCRIPT YANG DIPAKAI
-- ============================

-- Function to check if target is visible (wall check)
local function isTargetVisible(targetPart)
    if not targetPart then return false end
    if not Aimbot.WallCheckEnabled then return true end  -- Skip wall check if disabled
    ...
    -- Raycast from player's head to target
    ...
    if raycastResult then
        ...
        -- Check if what we hit is the target or part of the target
        if hitPart and hitPart:IsDescendantOf(targetPart.Parent) then
            return true
        end
        -- If we hit something else (like a wall), target is not visible
        return false
    end
    
    return true  -- If raycast hits nothing, target is visible
end
...
local function playerHasWeapon()
    if not Aimbot.InstantShot.RequireWeapon then return true end
    
    local character = LocalPlayer.Character
    if not character then return false end
    
    -- Cek jika player memegang tool (senjata)
...
local function getClosestEnemy()
    ...
    for _, enemy in pairs(Players:GetPlayers()) do
        if Aimbot.TeamCheck and enemy.Team == LocalPlayer.Team then continue end
        if isEnemy(enemy) then
            local root = enemy.Character:FindFirstChild("HumanoidRootPart")
            if root then
                -- Only consider enemies that are visible if wall check is enabled
                if not Aimbot.WallCheckEnabled or isTargetVisible(root) then
                    local dist = (root.Position - myHRP.Position).Magnitude
                    if dist < smallest then
                        smallest = dist
                        closest = enemy.Character
                        closestPosition = root.Position
                    end
                end
            end
        end
...
-- INSTANT REACTION SHOT FUNCTIONS
local function isTargetInCrosshair(target)
    ...
    -- Cek jika target berada dalam FOV
    local targetPart = target:FindFirstChild(Aimbot.AimPart) or target:FindFirstChild("HumanoidRootPart")
    if not targetPart then return false end
    
    -- Hitung posisi target di viewport
    local screenPos, onScreen = Camera:WorldToViewportPoint(targetPart.Position)
    if not onScreen then return false end
    
    -- Cek jika target berada di tengah layar (dalam radius FOV)
    local centerX = Camera.ViewportSize.X / 2
    local centerY = Camera.ViewportSize.Y / 2
    local distanceFromCenter = Vector2.new(screenPos.X, screenPos.Y) - Vector2.new(centerX, centerY)
    
    -- Gunakan FOV sebagai threshold
    local threshold = Aimbot.FOV
    return distanceFromCenter.Magnitude <= threshold
end

local function simulateMouseClick()
    -- Simulasi mouse click untuk menembak
    if UserInputService.MouseButton1Down then
        -- Jika mouse sudah ditekan, lepaskan dulu lalu tekan lagi
        local release = {UserInputType = Enum.UserInputType.MouseButton1, KeyCode = Enum.KeyCode.ButtonA}
        local press = {UserInputType = Enum.UserInputType.MouseButton1, KeyCode = Enum.KeyCode.ButtonA}
        
        -- Simulasi input untuk berbagai game
        pcall(function()
        ...
        pcall(function()
            -- Method 3: Coba tembak melalui remote events (untuk game shooting)
        ...
        -- Jika mouse belum ditekan, tekan saja
        pcall(function()
            game:GetService("VirtualInputManager"):SendMouseButtonEvent(0, 0, 0, true, game, 1)
            task.wait(0.05)
            game:GetService("VirtualInputManager"):SendMouseButtonEvent(0, 0, 0, false, game, 1)
...
local function performInstantShot()
    if not Aimbot.InstantShot.Enabled then return end
    if not Aimbot.Enabled then return end
    
    -- Cek delay antara tembakan
    local currentTime = tick()
    if currentTime - Aimbot.InstantShot.LastShotTime < Aimbot.InstantShot.ShotDelay then
        return
    end
    
    -- Cek apakah player punya senjata
    if not playerHasWeapon() then return end
    
    -- Cek apakah sedang aiming (jika OnlyWhenAiming diaktifkan)
    if Aimbot.InstantShot.OnlyWhenAiming and not (Aimbot.ToggleMode or UserInputService:IsMouseButtonPressed(Enum.UserInputType.MouseButton2)) then
        return
    end
    
    local target = getClosestEnemy()
    if target then
        -- Cek jika target berada dalam crosshair
        if isTargetInCrosshair(target) then
            -- Cek jarak maksimum
            local myHRP = LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart")
            local targetHRP = target:FindFirstChild("HumanoidRootPart")
            if myHRP and targetHRP then
                local distance = (myHRP.Position - targetHRP.Position).Magnitude
                if distance <= Aimbot.InstantShot.MaxDistance then
                    -- Tunggu reaction time
                    task.wait(Aimbot.InstantShot.ReactionTime)
                    
                    -- Lakukan tembakan
                    simulateMouseClick()
                    
                    -- Update waktu tembakan terakhir
                    Aimbot.InstantShot.LastShotTime = tick()
                end
            end
        end
    end
...
local function mainAimbotLoop()
    ...
    if target then
        local aimPart = target:FindFirstChild(Aimbot.AimPart)
        if not aimPart then
        ...
        if aimPart then
            ...
            -- Only aim if target is visible (when wall check is enabled)
            if not Aimbot.WallCheckEnabled or isTargetVisible(aimPart) then
                if Aimbot.Smoothness <= 1 then
                    instantAim(aimPos)
                else
                    setAim(aimPos)
                end
                if Aimbot.BlatantMode then
                    instantAim(aimPos)
                end
            end
        end
...
-- Connection untuk Instant Reaction Shot
local instantShotConnection
...
RunService.Heartbeat:Connect(function()
    ...
    if not aimbotConnection then
        aimbotConnection = RunService.Heartbeat:Connect(function()
            if Aimbot.Enabled and (Aimbot.ToggleMode or UserInputService:IsMouseButtonPressed(Enum.UserInputType.MouseButton2)) then
...
-- ============================
-- UI DENGAN TITLE BARU
-- ============================

Library:Watermark("SHOT or DIE | V1.0 (WT)")
local Window = Library:Window("SHOT or DIE | V1.0 (WT)")
...
Page:Groupbox("Aimbot Control", "Left"):AddToggle({
    Title = "Toggle Mode",
    Default = false,
    Description = "ON / OFF mode for mobile",
...
Page:Groupbox("Aimbot Control", "Left"):AddToggle({
    Title = "Blatant Mode",
    Default = false,
    Description = "Instant aim (no smoothness)",
    Callback = function(v) Aimbot.BlatantMode = v end
})
...
Page:Groupbox("Checks", "Left"):AddToggle({
    Title = "Wall Check",
    Default = true,
    Callback = function(v) Aimbot.WallCheckEnabled = v end
})
...
-- INSTANT REACTION SHOT SETTINGS
local InstantShotGroup = Page:Groupbox("Instant Reaction Shot", "Right")

InstantShotGroup:AddToggle({
    Title = "Enable Instant Shot",
    Default = false,
    Callback = function(v)
        Aimbot.InstantShot.Enabled = v
        if v and not instantShotConnection then
            instantShotConnection = RunService.Heartbeat:Connect(function()
                if Aimbot.InstantShot.Enabled then
                    pcall(performInstantShot)
                end
            end)
        elseif not v and instantShotConnection then
            instantShotConnection:Disconnect()
            instantShotConnection = nil
        end
    end
})
...
InstantShotGroup:AddToggle({
    Title = "Auto Shoot",
    Default = false,
    Callback = function(v) Aimbot.InstantShot.AutoShoot = v end
})

InstantShotGroup:AddToggle({
    Title = "Require Weapon",
    Default = true,
    Callback = function(v) Aimbot.InstantShot.RequireWeapon = v end
})

InstantShotGroup:AddToggle({
    Title = "Only When Aiming",
    Default = false,
    Callback = function(v) Aimbot.InstantShot.OnlyWhenAiming = v end
})

InstantShotGroup:AddSlider({
    Title = "Reaction Time",
    Min = 0.01,
    Max = 1.0,
    Default = 0.1,
    Suffix = " sec",
    Callback = function(v) Aimbot.InstantShot.ReactionTime = v end
})

InstantShotGroup:AddSlider({
    Title = "Max Shot Distance",
    Min = 50,
    Max = 2000,
    Default = 500,
    Suffix = " studs",
    Callback = function(v) Aimbot.InstantShot.MaxDistance = v end
})
...
InstantShotGroup:AddSlider({
    ...
    Default = 0.2,
    Suffix = " sec",
    Callback = function(v) Aimbot.InstantShot.ShotDelay = v end
})
...
Page:Groupbox("Smoothness", "Right"):AddSlider({
    Title = "Smoothness",
